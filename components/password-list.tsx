...
import { PasswordCredentialCard } from "./password-credential-card"

interface PasswordListProps {
  passwords: (CardCredential | PasswordCredential | SSHKeyCredential)[]
  folders: FolderItem[]
}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {passwords.map((password) => {
          if ("password" in password) {
            return <PasswordCredentialCard credential={password} />
          } else if ("card_number" in password) {
            return <p>Card</p>
          } else {
            return <p>SSH key</p>
          }
